...
export async function getAzureAccountExtensionApi(): Promise<AzureAccountExtensionApi> {
	const extension =
		vscode.extensions.getExtension<apiUtils.AzureExtensionApiProvider>(
	...
	if (extension) {
		...
		if ("getApi" in extension.exports) {
			azureAccountApi =
				extension.exports.getApi<AzureAccountExtensionApi>("1");
		} else {
			// support versions of the Azure Account extension <0.10.0
			azureAccountApi =
				extension.exports as unknown as AzureAccountExtensionApi;
		}
