...
export class StorageAccountResolver implements AppResourceResolver {
	...
	private storageAccountCache: Map<string, StorageAccountTreeItem> = new Map<
		string,
		StorageAccountTreeItem
	>();
	...
	public async resolveResource(
	...
	): Promise<ResolvedStorageAccount | undefined> {
		return await callWithTelemetryAndErrorHandling(
			...
			async (context: IActionContext) => {
				...
				if (
					this.storageAccountCacheLastUpdated <
					Date.now() - 1000 * 2
				) {
					this.storageAccountCache.clear();
					const storageAccounts = await uiUtils.listAllIterator(
					...
					const promises = storageAccounts.map(async (sa) => {
						if (sa.provisioningState !== "Succeeded") {
							...
							this.storageAccountCache.delete(
								nonNullProp(sa, "id"),
							);
						} else {
							const ti =
								await StorageAccountTreeItem.createStorageAccountTreeItem(
									subContext,
									new StorageAccountWrapper({
										...resource,
										...sa,
									}),
									storageManagementClient,
								);
							this.storageAccountCache.set(
								nonNullProp(sa, "id"),
								ti,
							);
						}
