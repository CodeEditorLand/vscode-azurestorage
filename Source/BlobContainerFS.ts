...
import {
	BlobServiceClient,
	StorageSharedKeyCredential as StorageSharedKeyCredentialBlob,
	type BlobClient,
...
import {
	...
	StorageSharedKeyCredential as StorageSharedKeyCredentialDataLake,
...
export class BlobContainerFS implements vscode.FileSystemProvider {
	...
	static constructUri(
		...
		return vscode.Uri.parse(
			`azurestorageblob:///${containerName}/${blobPath ?? ""}?storageAccountId=${storageAccountId}`,
		);
	}
	...
	private getBlobPath(uri: vscode.Uri): string {
		const pathWithoutContainer = uri.path.replace(
	 ...
	 */
	private async getStorageAccount(
		uri: vscode.Uri,
		...
		const keyResult = await client.storageAccounts.listKeys(
			resourceGroupName,
			storageAccountName,
		);
	...
	private async getBlobClients(
		uri: vscode.Uri,
		storageAccount: StorageAccount,
		accountKey?: string,
	): Promise<{ containerClient: ContainerClient; blobClient: BlobClient }> {
	...
	private async getDataLakeClients(
		uri: vscode.Uri,
		storageAccount: StorageAccount,
		accountKey?: string,
	): Promise<{
	...
	static async showEditor(
		context: IActionContext,
		treeItem: BlobTreeItem,
	): Promise<void> {
		const client: BlockBlobClient = await createBlockBlobClient(
			treeItem.root,
	...
	async stat(uri: vscode.Uri): Promise<vscode.FileStat> {
		...
		const result =
			await callWithTelemetryAndErrorHandling<vscode.FileStat | void>(
				"azureStorageBlob.stat",
				async (context) => {
						...
						if (!isHnsEnabled) {
							try {
								...
								return {
									...
									mtime:
										properties.lastModified?.getTime() ?? 0,
	...
	async readDirectory(uri: vscode.Uri): Promise<[string, vscode.FileType][]> {
		...
		>("azureStorageBlob.readDirectory", async (context) => {
			context.errorHandling.rethrow = true;

			const { storageAccount, accountKey } = await this.getStorageAccount(
				uri,
				context,
			);
			...
			do {
				...
				const listResult = containerClient
					...
					.byPage({
						maxPageSize: BlobContainerFS.listBlobPageSize,
						continuationToken,
					});

				const response = (await listResult.next())
					.value as ListBlobsHierarchySegmentResponse;

				response.segment.blobItems.forEach((childBlob) =>
					results.push([
						BlobPathUtils.basename(childBlob.name),
						vscode.FileType.File,
					]),
				);
	...
	async createDirectory(uri: vscode.Uri): Promise<void> {
		const result = await callWithTelemetryAndErrorHandling<void>(
				...
				if (!isHnsEnabled) {
					throw new Error(
						localize(
							...
							"Creating directories is not supported for non-HNS enabled storage accounts.",
						),
					);
				} else {
					const { pathClient } = await this.getDataLakeClients(
						uri,
						storageAccount,
						accountKey,
					);
						...
						async (progress) => {
							...
							await blobClient
								...
								.uploadData(content, {
									blobHTTPHeaders: {
										blobContentType:
											mime.getType(uri.path) || undefined,
									},
								});
						},
					);
	...
	async delete(
		...
		const result = await callWithTelemetryAndErrorHandling<void>(
			"azureStorageBlob.delete",
			async (context) => {
				context.errorHandling.rethrow = true;

				if (!options.recursive) {
					throw new Error(
						"Azure storage does not support non-recursive deletion.",
					);
				}
								...
								if (exists) {
									...
									do {
										...
										const listResult = containerClient
											...
											.byPage({
												maxPageSize:
													BlobContainerFS.listBlobPageSize,
												continuationToken,
											});

										const response = (
											await listResult.next()
										).value as ListBlobsFlatSegmentResponse;

										const deletePromiseBatch =
											response.segment.blobItems.map(
												(childBlob) => {
