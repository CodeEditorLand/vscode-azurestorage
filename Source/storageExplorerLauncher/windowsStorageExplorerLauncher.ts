...
const regKey: { hive: string; key: string } = {
	hive: "HKCR",
	key: "\\storageexplorer\\shell\\open\\command",
};

export class WindowsStorageExplorerLauncher
	implements IStorageExplorerLauncher
{
	...
	private static async getStorageExplorerExecutable(): Promise<string> {
		return (
			(await callWithTelemetryAndErrorHandling(
				"getStorageExplorerExecutableWindows",
				async (context) => {
					let regVal: string | undefined;

					try {
						regVal =
							await WindowsStorageExplorerLauncher.getWindowsRegistryValue(
								regKey.hive,
								regKey.key,
							);
					} catch (err) {
	...
	private static async getWindowsRegistryValue(
		hive: string,
		key: string,
		...
		return new Promise((resolve, reject) => {
			...
			rgKey.values((err?: {}, items?: Winreg.RegistryItem[]) => {
				if (err) {
					...
					resolve(
						items && items.length > 0 ? items[0].value : undefined,
					);
				}
			});
		});
	...
	private static async launchStorageExplorer(
		args: string[] = [],
	): Promise<void> {
		const storageExplorerExecutable: string =
			await WindowsStorageExplorerLauncher.getStorageExplorerExecutable();
