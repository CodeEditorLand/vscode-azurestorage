...
export async function checkCanOverwrite(
	...
	if (await destPathExists()) {
		if (overwriteChoice.choice === OverwriteChoice.noToAll) {
			...
			overwriteChoice.choice = await showDuplicateResourceWarning(
				context,
				destPath,
			);
...
async function showDuplicateResourceWarning(
	context: IActionContext,
	resourceName: string,
	...
	const items = [
		{
			...
			data: OverwriteChoice.yesToAll,
		},
		{ title: DialogResponses.yes.title, data: OverwriteChoice.yes },
		{
			title: localize("noToAll", "No to all"),
			data: OverwriteChoice.noToAll,
		},
