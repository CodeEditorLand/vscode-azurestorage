...
export async function loadMoreBlobChildren(
...
export async function createChildAsNewBlockBlob(
	parent: BlobContainerTreeItem | BlobDirectoryTreeItem,
	context: ICreateChildImplContext & IBlobContainerCreateChildContext,
): Promise<BlobTreeItem> {
	const blobPath: string =
		context.childName ?? (await getBlobPath(context, parent));
...
export async function createOrUpdateBlockBlob(
	parent: BlobContainerTreeItem | BlobDirectoryTreeItem,
	name: string,
	text?: string | Buffer,
): Promise<void> {
...
export async function doesBlobExist(
	treeItem: BlobContainerTreeItem | BlobDirectoryTreeItem,
	blobPath: string,
): Promise<boolean> {
...
export async function doesBlobDirectoryExist(
	treeItem: BlobContainerTreeItem | BlobDirectoryTreeItem,
	...
	for await (const responseValue of response) {
		if (
			responseValue.segment.blobItems.length ||
			responseValue.segment.blobPrefixes?.length
		) {
			return true;
		}
	}
...
export async function getExistingProperties(
	parent: BlobTreeItem | BlobContainerTreeItem | BlobDirectoryTreeItem,
	blobPath: string,
): Promise<BlockBlobUploadOptions | undefined> {
	const blockBlobClient: BlockBlobClient = await createBlockBlobClient(
		parent.root,
		parent.container.name,
		blobPath,
	);
...
export async function getBlobPath(
	...
	return await context.ui.showInputBox({
		...
		placeHolder: localize(
			"enterNameForNewBlockBlob",
			"Enter a name for the new block blob",
		),
		validateInput: async (name: string) => {
			...
			if (nameError) {
				...
				return localize(
					"aBlobWithThisPathAndNameAlreadyExists",
					"A blob with this path and name already exists",
				);
			}
