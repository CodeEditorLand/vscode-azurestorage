...
export function validateFileOrDirectoryName(name: string): string | undefined {
	...
	if (name.length < validLength.min || name.length > validLength.max) {
		return localize(
			"nameMustContain",
			"Name must contain between {0} and {1} characters",
			validLength.min,
			validLength.max,
		);
	}
	...
	if (invalidFileAndDirectoryChars.some((ch) => name.indexOf(ch) >= 0)) {
		return localize(
			"nameCantContain",
			"Name cannot contain the following characters: {0}",
			invalidFileAndDirectoryCharsString,
		);
	}
...
export function validateDocumentPath(
	...
	if (documentType === DocumentType.index && documentPath.includes("/")) {
		return localize(
			"indexDocumentPathCannotContainForwardSlash",
			'The index document path cannot contain a "/" character.',
		);
	} else if (
		documentType === DocumentType.error &&
		(documentPath.startsWith("/") || documentPath.endsWith("/"))
	) {
		return localize(
			"errorDocumentCannotStartOrEndWithForwardSlash",
			'The error document path start or end with a "/" character.',
		);
	} else if (
		documentPath.length < minLengthDocumentPath ||
		documentPath.length > maxLengthDocumentPath
	) {
		return localize(
			"documentPathLengthIsInvalid",
			"The {0} document path must be between {1} and {2} characters in length.",
			documentType,
			minLengthDocumentPath,
			maxLengthDocumentPath,
		);
	}
