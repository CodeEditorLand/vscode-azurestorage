...
import { uploadFolder, UploadItem } from "../commands/transfers/transfers";
...
export async function uploadLocalFolder(
	...
	const uploadItem: UploadItem = {
		...
		resourceName:
			destTreeItem instanceof BlobContainerTreeItem
				? destTreeItem.container.name
				: destTreeItem.shareName,
...
export function getUploadingMessageWithSource(
	sourcePath: string,
...
export function outputAndCopyUploadedFileUrls(
	parentUrl: string,
	fileUrls: string[],
): void {
	for (const fileUrl of fileUrls) {
		const url: string = localize(
			"uploadedFileUrl",
			"Uploaded file URL: {0}",
			`${parentUrl}/${fileUrl}`,
		);
	...
	const copyToClipboard: string = localize(
		"copyToClipboard",
		"Copy to Clipboard",
	);
...
export async function checkCanUpload(
	...
	return await checkCanOverwrite(
		context,
		destPath,
		overwriteChoice,
		async () => {
			if (treeItem instanceof BlobContainerTreeItem) {
				return (
					(await doesBlobExist(treeItem, destPath)) ||
					(await doesBlobDirectoryExist(treeItem, destPath))
				);
			} else {
				return (
					(await doesFileExist(
...
export function convertLocalPathToRemotePath(
	localPath: string,
...
export async function promptForDestinationDirectory(
	context: IActionContext,
): Promise<string> {
	return await context.ui.showInputBox({
		...
		prompt: localize(
			...
			"Enter the destination directory for this upload.",
		),
	});
}

export async function getDestinationDirectory(
	context: IActionContext,
	destinationDirectory?: string,
): Promise<string> {
	return destinationDirectory !== undefined
		? destinationDirectory
