...
export async function askAndCreateChildDirectory(
...
export async function listFilesInDirectory(
	...
	const response: AsyncIterableIterator<DirectoryListFilesAndDirectoriesSegmentResponse> =
		directoryClient
			...
			.byPage({ continuationToken: currentToken, maxPageSize });

	// eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
	const responseValue: DirectoryListFilesAndDirectoriesSegmentResponse = (
		await response.next()
	).value;
...
export async function deleteDirectoryAndContents(
	directory: string,
	shareName: string,
	root: IStorageRoot,
): Promise<void> {
	...
	while (true) {
		...
		for (const dir of directories) {
			await deleteDirectoryAndContents(
				path.posix.join(directory, dir.name),
				shareName,
				root,
			);
		}
...
export async function doesDirectoryExist(
	parent: FileShareTreeItem | DirectoryTreeItem,
