...
import {
	DirectoryTreeItem,
	IDirectoryDeleteContext,
} from "./tree/fileShare/DirectoryTreeItem";
import {
	FileShareTreeItem,
	IFileShareCreateChildContext,
} from "./tree/fileShare/FileShareTreeItem";
 ...
 */
export class AzureStorageFS
	implements vscode.FileSystemProvider, vscode.TextDocumentContentProvider
{
	...
	private _queryCache: Map<string, { query: string; invalid?: boolean }> =
		new Map<string, { query: string; invalid?: boolean }>(); // Key: rootName
	readonly onDidChangeFile: vscode.Event<vscode.FileChangeEvent[]> =
		this._emitter.event;

	static isAttachedAccount(
		treeItem: BlobContainerTreeItem | FileShareTreeItem | BlobTreeItem,
	): boolean {
	...
	static idToUri(resourceId: string, filePath?: string): vscode.Uri {
		...
		if (resourceId.startsWith("/attachedStorageAccounts")) {
			...
			if (/\/subscriptions\/.*\/subscriptions\//.test(resourceId)) {
				...
				idRegExp =
					/(\/subscriptions\/.*\/subscriptions\/[^\/]+\/resourceGroups\/[^\/]+\/providers\/Microsoft.Storage\/storageAccounts\/[^\/]+\/[^\/]+\/[^\/]+)\/?(.*)/i;
			} else {
				// resource groups v2
				idRegExp =
		...
		return vscode.Uri.parse(
			`azurestorage:///${path.posix.join(rootName, filePath)}?resourceId=${rootId}`,
		);
	}
	...
	public static fireDeleteEvent(node: AzExtTreeItem): void {
		ext.azureStorageFS.fireSoon({
			uri: AzureStorageFS.idToUri(node.fullId),
			...
			(await callWithTelemetryAndErrorHandling(
				...
				async (context) => {
					...
					const fileType: vscode.FileType =
						...
						treeItem instanceof BlobDirectoryTreeItem ||
		...
		return (
			(await callWithTelemetryAndErrorHandling(
				...
				async (context) => {
					...
					const loadingMessage: string = localize(
						"loadingDir",
						'Loading directory "{0}"...',
						treeItem.label,
					);
				...
				try {
					...
					if (response) {
						...
						throw getFileSystemError(uri, context, () => {
							return new vscode.FileSystemError(response);
						});
					}

					this.isFileShareUri(uri)
						? await this.createDirectoryFileShare(
								parsedUri,
								context,
							)
						: await this.createDirectoryBlobContainer(
								uri,
								parsedUri,
								context,
							);
	...
	async createDirectoryFileShare(
		parsedUri: IParsedUri,
		context: IActionContext,
	): Promise<void> {
		const parentUri: vscode.Uri = AzureStorageFS.idToUri(
		...
		await parent.createChild(<IFileShareCreateChildContext>{
			...
			childName: parsedUri.baseName,
		});
	}

	async createDirectoryBlobContainer(
		uri: vscode.Uri,
		parsedUri: IParsedUri,
		context: IActionContext,
	): Promise<void> {
		let treeItem: AzureStorageBlobTreeItem = await this.lookupBlobContainer(
			uri,
			context,
			parsedUri.resourceId,
			parsedUri.filePath,
		...
		const tiParsedUri = this.parseUri(
		...
		await callWithTelemetryAndErrorHandling(
			...
			async (context) => {
				...
				if (uri.path.endsWith("/")) {
					...
					throw new Error(
						"File/blob names with a trailing '/' are not allowed.",
					);
				}
				...
				if (treeItem instanceof FileShareTreeItem) {
					...
					childExistsRemote = await doesBlobExist(
						treeItem,
						parsedUri.filePath,
					);
				}
				...
				if (childExists !== childExistsRemote) {
					...
					throw new Error(
						localize(
							"outOfSync",
							"Your Azure Storage file system is out of sync and must be refreshed.",
						),
					);
				}
				...
				if (!childExists && !options.create) {
					...
					await vscode.window.withProgress(
						...
						async (progress) => {
							if (childExists) {
								progress.report({
									message: `Saving ${writeToFileShare ? "file" : "blob"} ${parsedUri.filePath}`,
								});

								if (treeItem instanceof FileShareTreeItem) {
									await updateFileFromText(
										parsedUri.parentDirPath,
										parsedUri.baseName,
										treeItem.shareName,
										treeItem.root,
								...
								if (writeToFileShare) {
									...
									>{
										...context,
										childType: FileTreeItem.contextValue,
										childName: parsedUri.baseName,
									});
								} else {
									await parent.createChild(<
									...
									>{
										...context,
										childType: BlobTreeItem.contextValue,
										childName: parsedUri.filePath,
	...
	async delete(
		...
		await callWithTelemetryAndErrorHandling("delete", async (context) => {
			...
			if (!options.recursive) {
				throw new Error(
					"Azure storage does not support nonrecursive deletion of folders.",
				);
			}
			...
			await vscode.window.withProgress(
				...
				async (progress) => {
					if (
						treeItem instanceof FileTreeItem ||
						treeItem instanceof DirectoryTreeItem ||
						treeItem instanceof BlobTreeItem ||
						treeItem instanceof BlobDirectoryTreeItem
					) {
						if (!(treeItem instanceof BlobDirectoryTreeItem)) {
							...
							progress.report({
								message: `Deleting ${parsedUri.filePath}`,
							});
						}

						await treeItem.deleteTreeItem(<IDirectoryDeleteContext>{
							...context,
							suppressMessage: true,
						});
					} else {
						throw new RangeError(
							`Unexpected entry ${treeItem.constructor.name}.`,
						);
					}
				},
			);
			...
			if (oldUriParsed.baseName === newUriParsed.baseName) {
				throw getFileSystemError(oldUri, context, () => {
					return new vscode.FileSystemError(
						"Moving folders or files is not supported.",
					);
				});
			} else {
				throw getFileSystemError(oldUri, context, () => {
					return new vscode.FileSystemError(
						"Renaming folders or files is not supported.",
					);
				});
			}
		});
	...
	async lookup(
		...
		return this.isFileShareUri(uri)
			? await this.lookupFileShare(uri, context, resourceId, filePath)
			: await this.lookupBlobContainer(
					uri,
					context,
					resourceId,
					filePath,
	...
	private async lookupFileShare(
		uri: vscode.Uri,
		context: IActionContext,
		resourceId: string,
		filePath: string,
	): Promise<AzureStorageFileTreeItem> {
		...
		const treeItem = resourceId.includes("attachedStorageAccounts")
			...
			: await ext.rgApi.appResourceTree.findTreeItem(uriPath, {
					...context,
					loadAll: true,
				});
		if (!treeItem) {
	...
	private async lookupBlobContainer(
		...
		resourceId: string,
		filePath: string,
		endSearchEarly?: boolean,
	): Promise<AzureStorageBlobTreeItem> {
	...
	private async lookupRoot(
		...
		const loadingMessage: string = this.isFileShareUri(uri)
			? localize(
					"loadingFileShare",
					'Loading file share "{0}"...',
					rootName,
				)
			: localize(
					"loadingContainer",
					'Loading blob container "{0}"...',
					rootName,
				);
		...
		const treeItem = resourceId.includes("attachedStorageAccounts")
			? await ext.rgApi.workspaceResourceTree.findTreeItem(resourceId, {
					...context,
					loadAll: true,
					loadingMessage,
				})
	...
	private async lookupAsDirectory(
		uri: vscode.Uri,
		context: IActionContext,
		resourceId?: string,
		filePath?: string,
	): Promise<AzureStorageDirectoryTreeItem> {
		const treeItem: AzureStorageTreeItem = await this.lookup(
			uri,
			context,
			resourceId,
		...
		if (cache) {
			if (uri.query) {
				this._queryCache.set(rootName, {
					query: cache.query,
					invalid: cache.invalid || cache.query !== uri.query,
				});
			} else {
				if (cache.invalid) {
					throw new Error(
						"Cannot auto-detect resource path. For this functionality, re-load VS Code and only open one resource per workspace.",
					);
				}

				// Fallback to the cached query because this uri's query doesn't exist
				return vscode.Uri.parse(
					`azurestorage://${uri.path}?${cache.query}`,
				);
			}
		} else {
			if (uri.query) {
