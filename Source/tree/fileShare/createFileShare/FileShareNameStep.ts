...
export class FileShareNameStep extends AzureWizardPromptStep<IFileShareWizardContext> {
	...
	private validateFileShareName(name: string): string | undefined | null {
		...
		if (!name) {
			...
			return localize(
				"shareNameSpaces",
				"Share name cannot contain spaces",
			);
		} else if (
			name.length < validLength.min ||
			name.length > validLength.max
		) {
			return localize(
				"shareNameBetween",
				"Share name must contain between {0} and {1} characters",
				validLength.min,
				validLength.max,
			);
		} else if (!/^[a-z0-9-]+$/.test(name)) {
			return localize(
				"shareNameInvalidChar",
				"Share name can only contain lowercase letters, numbers and hyphens",
			);
		} else if (/--/.test(name)) {
			return localize(
				"shareNameDoubleHyphen",
				"Share name cannot contain two hyphens in a row",
			);
		} else if (/(^-)|(-$)/.test(name)) {
			return localize(
				"shareNameHyphen",
				"Share name cannot begin or end with a hyphen",
			);
		}
