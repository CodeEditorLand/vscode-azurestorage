...
export class FileShareGroupTreeItem
	...
	async loadMoreChildrenImpl(
		...
		try {
			...
			if (parseError(error).errorType === "REQUEST_SEND_ERROR") {
				throw new Error(
					localize(
						"storageAccountDoesNotSupportFileShares",
						"This storage account does not support file shares.",
					),
				);
			} else {
				throw error;
		...
		return shares.map((share: ShareItem) => {
			return new FileShareTreeItem(
				this,
				share.name,
				shareServiceClient.getShareClient(share.name).url,
			);
		});
	}
		...
		return await window.withProgress(
			...
			async (progress) => {
				...
				progress.report({
					message: localize(
						"creatingFileShare",
						'Azure Storage: Creating file share "{0}"...',
						shareName,
					),
				});
				const shareServiceClient: ShareServiceClient =
				...
				return new FileShareTreeItem(
					this,
					shareName,
					shareServiceClient.getShareClient(shareName).url,
				);
			},
		);
	}
