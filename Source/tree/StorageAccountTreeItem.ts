...
import {
	ShareServiceClient,
	StorageSharedKeyCredential as StorageSharedKeyCredentialFileShare,
} from "@azure/storage-file-share";
import {
	QueueServiceClient,
	StorageSharedKeyCredential as StorageSharedKeyCredentialQueue,
...
export type ResolvedStorageAccountTreeItem =
	ResolvedAppResourceTreeItem<ResolvedStorageAccount>;

export function isResolvedStorageAccountTreeItem(
	t: unknown,
): t is ResolvedStorageAccountTreeItem {
	return (
		(t as ResolvedStorageAccountTreeItem)?.kind?.toLowerCase() ===
		StorageAccountTreeItem.kind
	);
}

export class StorageAccountTreeItem
	implements ResolvedStorageAccount, IStorageTreeItem
{
	...
	public static async createStorageAccountTreeItem(
		subscription: ISubscriptionContext,
		storageAccount: StorageAccountWrapper,
		client: StorageManagementClient,
	): Promise<StorageAccountTreeItem> {
		const ti = new StorageAccountTreeItem(
			subscription,
			storageAccount,
			client,
		);
	...
	async loadMoreChildrenImpl(_clearCache: boolean): Promise<AzExtTreeItem[]> {
		this._blobContainerGroupTreeItem = new BlobContainerGroupTreeItem(
	...
	public async deleteTreeItemImpl(context: IActionContext): Promise<void> {
		const deletingStorageAccount: string = localize(
	...
	private createRoot(): IStorageRoot {
		return {
			...
			createBlobServiceClient: async () => {
				const credential = new StorageSharedKeyCredentialBlob(
					this.storageAccount.name,
					this.key.value,
				);
			...
			createShareServiceClient: async () => {
				const credential = new StorageSharedKeyCredentialFileShare(
					this.storageAccount.name,
					this.key.value,
				);
			...
			createQueueServiceClient: async () => {
				const credential = new StorageSharedKeyCredentialQueue(
					this.storageAccount.name,
					this.key.value,
				);
	...
	public static parseAzureResourceId(resourceId: string): {
		[key: string]: string;
	} {
	...
	public async getActualWebsiteHostingStatus(): Promise<WebsiteHostingStatus> {
		...
		return {
			...
			errorDocument404Path:
				staticWebsite && staticWebsite.errorDocument404Path,
		};
	}

	public async setWebsiteHostingProperties(
	...
	public async configureStaticWebsite(
		context: IActionContext,
	): Promise<void> {
		...
		const wizard: AzureWizard<IStaticWebsiteConfigWizardContext> =
			new AzureWizard(wizardContext, {
				promptSteps: [
					new StaticWebsiteIndexDocumentStep(oldStatus.indexDocument),
					new StaticWebsiteErrorDocument404Step(
						oldStatus.errorDocument404Path,
					),
				],
				executeSteps: [
					new StaticWebsiteConfigureStep(this, oldStatus.enabled),
				],
				title: localize(
	...
	public async browseStaticWebsite(context: IActionContext): Promise<void> {
		...
		if (endpoint) {
			...
			throw new Error(
				`Could not retrieve the primary web endpoint for ${this.label}`,
			);
		}
	}

	public async ensureHostingCapable(
		context: IActionContext,
		...
		if (!hostingStatus.capable) {
			...
			if (accountType !== "StorageV2") {
				...
				throw new Error(
					localize(
						...
						"Only general purpose V2 storage accounts support static website hosting.",
					),
				);
			}
			...
			throw new Error(
				localize(
					"doesntSupportHosting",
					"This storage account does not support static website hosting.",
				),
			);
		}
	}
