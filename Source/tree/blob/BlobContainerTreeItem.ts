...
import {
	...
	createChildAsNewBlockBlob,
	IBlobContainerCreateChildContext,
	loadMoreBlobChildren,
...
import {
	isResolvedStorageAccountTreeItem,
	ResolvedStorageAccountTreeItem,
	StorageAccountTreeItem,
} from "../StorageAccountTreeItem";
...
export class BlobContainerTreeItem
	...
	implements ICopyUrl, ITransferSrcOrDstTreeItem
{
	...
	public static async createBlobContainerTreeItem(
		parent: BlobContainerGroupTreeItem,
	...
	public async loadMoreChildrenImpl(
		...
		const { children, continuationToken } = await loadMoreBlobChildren(
			this,
	...
	public compareChildrenImpl(
		ti1: BlobContainerTreeItem,
		ti2: BlobContainerTreeItem,
	): number {
	...
	private async listAllBlobs(
		...
		ext.outputChannel.appendLog(
			`Querying Azure... Method: listBlobsFlat blobContainerName: "${this.container.name}" prefix: ""`,
		);
	...
	public async createChildImpl(
		...
		if (context.remoteFilePath && context.localFilePath) {
			...
			context.childType === BlobDirectoryTreeItem.contextValue
		) {
			const containerClient = await createBlobContainerClient(
				this.root,
				this.container.name,
	...
	public async deployStaticWebsite(
		...
		const webEndpoint = await vscode.window.withProgress(
			...
			async (notificationProgress, cancellationToken) =>
				await this.deployStaticWebsiteCore(
					context,
					sourceFolderPath,
					destBlobFolder,
					notificationProgress,
	...
	private async deployStaticWebsiteCore(
		...
		ext.outputChannel.appendLog(
			localize(
				"deploying",
				'Deploying to static website "{0}/{1}"',
			...
			async (currentAttempt) => {
				...
				if (
					getWorkspaceSetting<boolean>(
						configurationSettingsKeys.deleteBeforeDeploy,
					)
				) {
					// Find existing blobs
					...
					const transferProgress = new TransferProgress(
						"blobs",
						"Deleting",
					);
		...
		ext.outputChannel.appendLog(
			`Deployment to static website complete. Primary web endpoint is ${webEndpoint}`,
		);
	...
	public getStorageAccountTreeItem(
		treeItem: AzExtTreeItem,
	): ResolvedStorageAccountTreeItem & AzExtTreeItem {
		if (!(treeItem instanceof BlobContainerTreeItem)) {
			throw new Error(
				`Unexpected treeItem type: ${treeItem.contextValue}`,
			);
		}
		...
		if (
			...
			throw new Error(
				"Internal error: Couldn't find storage account treeItem for container",
			);
		}
	}
	...
	private async deleteBlobs(
		...
		for (const blobIndex of blobsToDelete.keys()) {
			...
			try {
				...
				throw new Error(
					`Error deleting blob "${blob.name}" : ${parseError(error).message}`,
				);
			}
		}
	}
