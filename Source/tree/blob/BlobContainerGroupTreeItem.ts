...
export class BlobContainerGroupTreeItem
	...
	implements IStorageTreeItem
{
	...
	public async loadMoreChildrenImpl(
		...
		try {
			...
			if (this.root.isEmulated && errorType === "ECONNREFUSED") {
				...
				throw new Error(
					localize(
						"storageAccountDoesNotSupportBlobs",
						"This storage account does not support blobs.",
					),
				);
			} else {
				throw error;
		...
		const result: AzExtTreeItem[] = await Promise.all(
			containersResponse.containerItems.map(
				async (container: ContainerItem) => {
	...
	async listContainers(
		...
		const response: AsyncIterableIterator<ServiceListContainersSegmentResponse> =
			blobServiceClient
				.listContainers()
				.byPage({ continuationToken, maxPageSize: maxPageSize });
