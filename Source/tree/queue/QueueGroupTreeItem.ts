...
export class QueueGroupTreeItem
	...
	async loadMoreChildrenImpl(clearCache: boolean): Promise<AzExtTreeItem[]> {
		...
		try {
			...
			if (this.root.isEmulated && errorType === "ECONNREFUSED") {
				...
				throw new Error(
					localize(
						"storageAccountDoesNotSupportQueues",
						"This storage account does not support queues.",
					),
				);
			} else {
				throw error;
		...
		return (
			queuesResponse.queueItems?.map((queue: QueueItem) => {
				return new QueueTreeItem(this, queue);
			}) || []
		);
	}
