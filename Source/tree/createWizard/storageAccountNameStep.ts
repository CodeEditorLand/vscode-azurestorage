...
export class StorageAccountNameStep<
	T extends IStorageAccountWizardContext,
> extends AzureNameStep<T> {
	public async prompt(context: T): Promise<void> {
		const client: StorageManagementClient =
			await createStorageClient(context);
	...
	private async validateStorageAccountName(
		...
		if (
			...
			name.length > storageAccountNamingRules.maxLength
		) {
			return `The name must be between ${storageAccountNamingRules.minLength} and ${storageAccountNamingRules.maxLength} characters.`;
		} else if (
			name.match(storageAccountNamingRules.invalidCharsRegExp) !== null
		) {
			return "The name can only contain lowercase letters and numbers.";
		} else {
			const nameAvailabilityResult: CheckNameAvailabilityResult =
				await client.storageAccounts.checkNameAvailability({
					name,
