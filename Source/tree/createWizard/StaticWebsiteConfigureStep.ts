...
export class StaticWebsiteConfigureStep extends AzureWizardExecuteStep<
...
> {
	...
	public async execute(
		wizardContext: IStorageAccountTreeItemCreateContext &
			IStaticWebsiteConfigWizardContext,
		progress: NotificationProgress,
	): Promise<void> {
		this.accountTreeItem =
			this.accountTreeItem || wizardContext.accountTreeItem;

		progress.report({
			message: localize(
				"configuringStaticWebsiteHosting",
				`Configuring static website hosing for storage account "${this.accountTreeItem.label}"...`,
			),
		});
		...
		let msg = this.previouslyEnabled
			? localize(
					"staticWebsiteHostingConfigurationUpdated",
					'Static website hosting configuration updated for storage account "{0}".',
					this.accountTreeItem.label,
				)
			: localize(
					"storageAccountHasBeenEnabledForStaticWebsiteHosting",
		...
		if (
			newStatus.staticWebsite &&
			this.previouslyEnabled !== newStatus.staticWebsite.enabled
		) {
			await ext.rgApi.appResourceTree.refresh(
				wizardContext,
				this.accountTreeItem as unknown as AzExtTreeItem,
			);
		}
	}

	public shouldExecute(
		wizardContext: IStaticWebsiteConfigWizardContext,
	): boolean {
