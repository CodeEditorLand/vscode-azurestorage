...
export class StaticWebsiteEnableStep extends AzureWizardPromptStep<IStaticWebsiteConfigWizardContext> {
	public async prompt(
		context: IStaticWebsiteConfigWizardContext,
	): Promise<void> {
		if (!context.isCustomCloud) {
			...
			context.enableStaticWebsite =
				(await context.ui.showQuickPick([yes, no], { placeHolder })) ===
				yes;
		} else {
	...
	): Promise<IWizardOptions<IStaticWebsiteConfigWizardContext> | undefined> {
		if (wizardContext.enableStaticWebsite) {
			return {
				promptSteps: [
					new StaticWebsiteIndexDocumentStep(),
