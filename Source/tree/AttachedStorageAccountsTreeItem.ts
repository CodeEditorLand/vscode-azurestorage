...
export class AttachedStorageAccountsTreeItem extends AzExtParentTreeItem {
	...
	public isAncestorOfImpl(contextValue: string): boolean {
		return (
			contextValue !== StorageAccountTreeItem.contextValue &&
			contextValue !== SubscriptionTreeItem.contextValue
		);
	}

	public async attachWithConnectionString(
		context: IActionContext,
	): Promise<void> {
		const connectionString = await context.ui.showInputBox({
			prompt: localize(
				"enterConnectionString",
				"Enter the connection string for your storage account",
			),
			validateInput: (value: string): string | undefined =>
				this.validateConnectionString(value),
		});

		const accountName: string =
			getPropertyFromConnectionString(connectionString, "AccountName") ||
			emulatorAccountName;
	...
	public async detach(
		treeItem: AttachedStorageAccountTreeItem,
	): Promise<void> {
		const attachedAccounts: AttachedStorageAccountTreeItem[] =
	...
	private async getAttachedAccounts(): Promise<
		AttachedStorageAccountTreeItem[]
	> {
			...
			try {
				...
				throw new Error(
					localize(
						...
						"Failed to load persisted Storage Accounts. Accounts must be reattached manually.",
					),
				);
			}
		}
	...
	private async attachAccount(
		...
		if (attachedAccounts.find((s) => s.fullId === treeItem.fullId)) {
			throw new Error(
				localize(
					"storageAccountIsAlreadyAttached",
					'Storage Account "{0}" is already attached.',
					treeItem.id,
				),
			);
		} else {
	...
	private async loadPersistedAccounts(): Promise<
		AttachedStorageAccountTreeItem[]
	> {
	...
	private validateConnectionString(
		...
		if (connectionString === emulatorConnectionString) {
			return localize(
				"emulatorAlreadyAttached",
				"Local emulator is already attached.",
			);
		}
		...
		return localize(
			"connectionStringMustMatchFormat",
			'Connection string must match format "DefaultEndpointsProtocol=...;AccountName=...;AccountKey=...;"',
		);
	}
