...
export class AttachedStorageAccountTreeItem
	...
	implements IStorageTreeItem
{
	...
	constructor(
		...
		this._root = new AttachedStorageRoot(
			connectionString,
			storageAccountName,
	...
	public get label(): string {
		return this.root.isEmulated
			? localize("localEmulator", "Local Emulator")
			: this.storageAccountName;
	}

	public get contextValue(): string {
		return this.root.isEmulated
			? AttachedStorageAccountTreeItem.emulatedContextValue
			: AttachedStorageAccountTreeItem.baseContextValue;
	}

	// eslint-disable-next-line @typescript-eslint/require-await
	public async loadMoreChildrenImpl(): Promise<AzExtTreeItem[]> {
		const groupTreeItems: AzExtTreeItem[] = [
			this._blobContainerGroupTreeItem,
			this._queueGroupTreeItem,
			this._tableGroupTreeItem,
		];
	...
	public isAncestorOfImpl(contextValue: string): boolean {
		return (
			contextValue !== AttachedStorageAccountTreeItem.baseContextValue ||
			!this.root.isEmulated
		);
	}
}
...
class AttachedStorageRoot extends AttachedAccountRoot {
	...
	public get storageAccountId(): string {
		throw new Error(
			localize(
				"cannotRetrieveStorageAccountIdForAttachedAccount",
				"Cannot retrieve storage account id for an attached account.",
			),
		);
	}
	...
	public generateSasToken(
		...
		const key: string | undefined =
			this._connectionString === emulatorConnectionString
				? emulatorKey
		...
		if (!key) {
			throw new Error(
				localize(
					"noKeyConnectionString",
					"Could not parse key from connection string",
				),
			);
		}
	...
	public async createBlobServiceClient(): Promise<BlobServiceClient> {
		return BlobServiceClient.fromConnectionString(
			this._connectionString,
			this._serviceClientPipelineOptions,
		);
	}
	...
	public async createTableServiceClient(): Promise<TableServiceClient> {
		return TableServiceClient.fromConnectionString(this._connectionString, {
			retryOptions: {
				maxRetries:
					this._serviceClientPipelineOptions.retryOptions.maxTries,
			},
		});
	}
}
