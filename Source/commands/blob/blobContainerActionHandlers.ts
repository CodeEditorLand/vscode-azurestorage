		...
		async (context: IActionContext, parent: BlobContainerTreeItem) => {
			...
			for (const dirName of dirNames) {
				const treeItem:
					...
					| undefined = await ext.rgApi.appResourceTree.findTreeItem(
				...
				if (!treeItem) {
					...
					>{
						...context,
						childType: BlobDirectoryTreeItem.contextValue,
						childName: dirName,
					});
				} else {
					if (treeItem instanceof BlobTreeItem) {
						throw new Error(
							localize(
								"resourceIsNotADirectory",
								`"${treeItem.blobPath}" is not a directory`,
							),
						);
					}
			...
			const childTreeItem: BlobTreeItem = <BlobTreeItem>(
				await dirParentTreeItem.createChild(<
					IBlobContainerCreateChildContext
				>{
					...context,
...
async function openBlobContainerInStorageExplorer(
...
export async function deleteBlobContainer(
