...
export async function uploadToAzureStorage(
	...
	_firstSelection: vscode.Uri,
	uris: vscode.Uri[],
): Promise<void> {
	const treeItem = await ext.rgApi.pickAppResource<
	...
	for (const uri of uris) {
		if (uri.scheme === "azurestorage") {
			throw new Error(
				localize(
					"cannotUploadToAzureFromAzureResource",
					"Cannot upload to Azure from an Azure resource.",
				),
			);
		}
	...
	await vscode.window.withProgress(
		...
		async (_notificationProgress, cancellationToken) => {
