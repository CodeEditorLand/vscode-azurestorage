...
import {
	IStorageAccountTreeItemCreateContext,
	StorageAccountTreeItemCreateStep,
} from "../tree/createWizard/StorageAccountTreeItemCreateStep";
...
export async function createStorageAccount(
	context: IActionContext & Partial<ICreateChildImplContext>,
	treeItem?: SubscriptionTreeItem,
): Promise<StorageAccountTreeItem> {
	if (!treeItem) {
		treeItem = <SubscriptionTreeItem>(
			await ext.rgApi.appResourceTree.showTreeItemPicker(
	...
	const wizardContext: IStorageAccountWizardContext & ExecuteActivityContext =
	...
	const executeSteps: AzureWizardExecuteStep<IStorageAccountWizardContext>[] =
		[
			new StorageAccountCreateStep({
				kind: wizardContext.isCustomCloud
	...
	if (context.advancedCreation) {
		...
		Object.assign(wizardContext, {
			...
			indexDocument: wizardContext.isCustomCloud
				? ""
				: StaticWebsiteIndexDocumentStep.defaultIndexDocument,
			errorDocument404Path: wizardContext.isCustomCloud
				? ""
				: StaticWebsiteErrorDocument404Step.defaultErrorDocument404Path,
		});
	}
	...
	LocationListStep.getQuickPickDescription = (location: AzExtLocation) => {
		return location.metadata?.regionCategory === "Extended"
			? localize(
					"onlyPremiumSupported",
					"Only supports Premium storage accounts",
				)
			: undefined;
	};
	...
	const newStorageAccountName = nonNullProp(
		wizardContext,
	...
	(<ISelectStorageAccountContext>context).showEnableWebsiteHostingPrompt =
		false;

	return (wizardContext as unknown as IStorageAccountTreeItemCreateContext)
		.accountTreeItem;
}

export async function createStorageAccountAdvanced(
	actionContext: IActionContext,
	treeItem?: SubscriptionTreeItem,
): Promise<StorageAccountTreeItem> {
	return await createStorageAccount(
		{ ...actionContext, advancedCreation: true },
		treeItem,
	);
}
