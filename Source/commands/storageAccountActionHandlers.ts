...
} from "../tree/StorageAccountTreeItem";
...
async function openStorageAccountInStorageExplorer(
	context: IActionContext,
	treeItem?: ResolvedAppResourceTreeItem<ResolvedStorageAccount>,
): Promise<void> {
	if (!treeItem) {
	...
	await storageExplorerLauncher.openResource(
		accountId,
		treeItem.subscription.subscriptionId,
	);
}

export async function copyPrimaryKey(
	context: IActionContext,
	treeItem?: StorageAccountTreeItem,
): Promise<void> {
	if (!treeItem) {
		treeItem = await pickStorageAccount(context);
	}
...
export async function deployStaticWebsite(
	...
	if (!sourcePath) {
		sourcePath = await showWorkspaceFoldersQuickPick(
			localize("selectFolderToDeploy", "Select the folder to deploy"),
			context,
			configurationSettingsKeys.deployPath,
		);
	}

	// Ask for destination account
	const destAccountTreeItem: ResolvedStorageAccountTreeItem =
		await selectStorageAccountTreeItemForCommand(destTreeItem, context, {
...
async function runPreDeployTask(
	deployFsPath: string,
	context: IActionContext,
): Promise<void> {
	...
	if (taskName) {
		...
		if (preDeployTask) {
							...
							(e: vscode.TaskProcessEndEvent) => {
								if (e.execution.task === preDeployTask) {
									...
									if (e.exitCode === 0) {
										...
										reject(
											new Error(
												`Pre-deploy task "${e.execution.task.name}" failed with exit code "${e.exitCode}".`,
											),
										);
									}
								}
			...
			throw new Error(
				`Failed to find pre-deploy task "${taskName}". Modify your tasks or the setting "${extensionPrefix}.${configurationSettingsKeys.preDeployTask}".`,
			);
		}
	}
}

function isTaskEqual(
	expectedName: string,
	expectedPath: string,
	actualTask: vscode.Task,
): boolean {
	if (
		actualTask.name &&
		actualTask.name.toLowerCase() === expectedName.toLowerCase() &&
		actualTask.scope !== undefined
	) {
		...
		return (
			...
			(isPathEqual(workspaceFolder.uri.fsPath, expectedPath) ||
				isSubpath(workspaceFolder.uri.fsPath, expectedPath))
		);
	} else {
		return false;
	}
