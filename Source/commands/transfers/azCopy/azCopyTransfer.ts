...
import {
	...
	ITransferStatus,
	ProgressJobStatus,
} from "@azure-tools/azcopy-node/dist/src/Output/TransferStatus";
...
function handleJob(
	...
	if (!finalTransferStatus || finalTransferStatus.JobStatus !== "Completed") {
		...
		if (
			...
			message += localize(
				"checkOutputWindow",
				" Check the [output window](command:{0}) for a list of incomplete transfers.",
			...
			ext.outputChannel.appendLog(
				localize(
					"couldNotTransfer",
					'Could not transfer "{0}"',
					transferLabel,
				),
			);

			if (
				finalTransferStatus?.JobStatus === "Failed" &&
				process.platform === "linux"
			) {
				message += localize(
					"viewHelp",
					" View help with [known issues](https://aka.ms/AAb0i6o).",
				);
			}
		}
		...
		if (
			finalTransferStatus?.JobStatus &&
			/CompletedWith*/gi.test(finalTransferStatus.JobStatus)
		) {
