...
export async function downloadFoldersAndFiles(
	...
	const foldersToDownload = (
		await Promise.all(
			folders
				...
				.map(async (folder) =>
					(await okToOverwriteOrDoesNotExist(
						context,
						folder,
						overwriteChoice,
					))
		...
		(maybeFolder): maybeFolder is DownloadItem => maybeFolder !== undefined,
	);
	...
	if (itemsToDownload.length > 0) {
		...
		const downloadedMessage: string = localize(
...
export async function uploadFile(
	...
	ext.outputChannel.appendLog(
		getUploadingMessageWithSource(item.localFilePath, item.resourceName),
	);

	if (!ext.isWeb) {
		// @todo: add code path for isWeb === true
		await startAzCopyFileUpload(
			context,
			item,
			notificationProgress,
			cancellationToken,
 ...
 */
function parentIsNotInFolderList(
	item: DownloadItem,
	folders: DownloadItem[],
): boolean {
	return !folders.some((maybeParent) => {
		return (
			item !== maybeParent &&
			isSubpath(maybeParent.remoteFilePath, item.remoteFilePath)
		);
	});
}
 ...
 */
function okToOverwriteOrDoesNotExist(
	context: IActionContext,
	item: DownloadItem,
	overwriteChoice: { choice: OverwriteChoice | undefined },
): Promise<boolean> {
	return checkCanOverwrite(
		context,
		item.localFilePath,
		overwriteChoice,
		async () => await AzExtFsExtra.pathExists(item.localFilePath),
	);
}

async function startAzCopyDownload(
	context: IDownloadWizardContext,
	item: DownloadItem,
	...
	const src: IRemoteSasLocation = createAzCopyRemoteLocation(
		item.resourceUri,
		item.sasToken,
		item.remoteFilePath,
		item.isDirectory,
...
async function startAzCopyFileUpload(
	context: IActionContext,
	item: UploadItem,
	notificationProgress?: NotificationProgress,
	cancellationToken?: CancellationToken,
...
async function startAzCopyFolderUpload(
	context: IActionContext,
	item: UploadItem,
	messagePrefix?: string,
	notificationProgress?: NotificationProgress,
	...
	const src: ILocalLocation = createAzCopyLocalLocation(
		item.localFilePath,
		useWildCard,
	);
	const dst: IRemoteSasLocation = createAzCopyRemoteLocation(
