...
export async function selectStorageAccountTreeItemForCommand(
	...
	if (treeItem?.parent?.parent instanceof AttachedStorageAccountTreeItem) {
		...
		throw new Error(
			localize(
				"staticWebsiteCommandsNotSupportedForAttachedAccounts",
				"Static website commands are not supported for attached accounts.",
			),
		);
	}
	...
	if (options.mustBeWebsiteCapable) {
		...
		if (options.configureWebsite && !hostingStatus.enabled) {
			context.telemetry.properties.cancelStep =
				"StorageAccountWebSiteNotEnabled";
