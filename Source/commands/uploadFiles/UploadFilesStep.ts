...
export class UploadFilesStep extends AzureWizardExecuteStep<IUploadFilesWizardContext> {
	...
	public async execute(
		context: Required<IUploadFilesWizardContext>,
		...
		if (notificationProgress && this.cancellationToken) {
			context.resolution = await uploadFilesStepHelper(
				context,
				context.treeItem,
				urisToUpload,
			...
			const resolution: IAzCopyResolution = await window.withProgress(
				...
				async (newNotificationProgress, newCancellationToken) => {
					return await uploadFilesStepHelper(
						context,
						nonNullValue(context.treeItem),
						urisToUpload,
						newNotificationProgress,
						newCancellationToken,
						nonNullValue(context.destinationDirectory),
						context.calledFromUploadToAzureStorage,
					);
				},
			);
