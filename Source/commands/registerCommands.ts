...
export function registerCommands(): void {
	...
	registerCommandWithTreeNodeUnwrapping(
		...
		async (
			...
			treeItem?: AzExtTreeItem & IStorageTreeItem,
		) => {
	...
	registerCommandWithTreeNodeUnwrapping(
		...
		async (
			...
			if (!treeItem) {
				treeItem = await ext.rgApi.pickAppResource<
					BlobContainerTreeItem | FileShareTreeItem
				>(actionContext, {
					filter: storageFilter,
					expectedChildContextValue: [
						BlobContainerTreeItem.contextValue,
	...
	registerCommandWithTreeNodeUnwrapping(
		...
		async (actionContext: IActionContext, treeItem?: AzExtTreeItem) => {
			const accountTreeItem =
				await selectStorageAccountTreeItemForCommand(
					treeItem,
					actionContext,
					{
	...
	registerCommandWithTreeNodeUnwrapping(
		...
		async (actionContext: IActionContext) => {
			await startEmulator(actionContext, EmulatorType.blob);
		},
		emulatorTimeoutMS,
	);
	registerCommandWithTreeNodeUnwrapping(
		...
		async (actionContext: IActionContext) => {
			await startEmulator(actionContext, EmulatorType.queue);
		},
		emulatorTimeoutMS,
	);
	registerCommandWithTreeNodeUnwrapping(
		...
		async (actionContext: IActionContext) => {
			await startEmulator(actionContext, EmulatorType.table);
		},
		emulatorTimeoutMS,
	);
	registerCommandWithTreeNodeUnwrapping(
		...
		async () => {
			await commands.executeCommand("extension.open", azuriteExtensionId);
		},
	);
}
