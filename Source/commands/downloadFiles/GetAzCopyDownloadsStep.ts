...
export class GetAzCopyDownloadsStep extends AzureWizardExecuteStep<IDownloadWizardContext> {
	...
	public async execute(
		...
		context.sasUrl
			...
			: await this.setDownloadItemsFromTreeItems(
					context,
					destinationFolder,
					context.treeItems,
				);
	...
	public shouldExecute(wizardContext: IDownloadWizardContext): boolean {
		return (
			!wizardContext.allFileDownloads && !wizardContext.allFolderDownloads
		);
	}

	private async setDownloadItemsFromTreeItems(
		context: IDownloadWizardContext,
	...
	private async setDownloadItemsFromContextSasUrl(
		context: IDownloadWizardContext,
		destinationFolder: string,
	): Promise<void> {
		const allFolderDownloads: DownloadItem[] = [];
