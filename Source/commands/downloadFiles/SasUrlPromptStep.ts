...
export class SasUrlPromptStep extends AzureWizardPromptStep<IActionContext> {
	public async prompt(context: ISasDownloadContext): Promise<void> {
		...
		context.sasUrl = await context.ui.showInputBox({
			...
			validateInput: async (s): Promise<string | undefined> =>
				await this.validateInput(context, s),
			value,
		});
	}
	...
	public async validateInput(
		_wizardContext: ISasDownloadContext,
		value: string | undefined,
	): Promise<string | undefined> {
		if (!value) {
			return localize(
				"emptySasUrl",
				"A SAS token and URL cannot be empty.",
			);
		}
		...
		try {
			...
			if (
				...
				return localize(
					"enterValidToken",
					"The SAS token is missing a parameter. Enter a valid SAS token.",
				);
			}
		} catch (err) {
			return localize(
				"enterValidSasUrl",
				'The URL "{0}" is not valid. Enter a valid URL',
				value,
			);
		}
